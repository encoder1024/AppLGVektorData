...
const getFreshnessWindowMs = (scanRateMs) => Math.max((scanRateMs || 1000) * 5, 60000);
...
class SystemHealthService {
  ...
  async start() {
    ...
    this.interval = setInterval(() => {
      this.runSnapshotCycle().catch((error) => {
        console.error('SystemHealthService: fallo en ciclo programado:', error.message);
      });
    }, HEALTH_INTERVAL_MS);

    console.log(`SystemHealthService: housekeeping iniciado (${HEALTH_INTERVAL_MS}ms).`);
  }
  ...
  async runSnapshotCycle() {
    ...
    try {
      ...
      const latestActuatorById = new Map(latestActuatorActions.map((row) => [row.actuator_id, row]));

      const sensorSnapshots = sensors.map((sensor) =>
        this.buildSensorSnapshot(sensor, latestSensorById.get(sensor.id), plcs, snapshotTime)
      );
      const actuatorSnapshots = actuators.map((actuator) =>
        this.buildActuatorSnapshot(actuator, latestActuatorById.get(actuator.id), plcs, snapshotTime)
      );
  ...
  async getLatestSensorReadings() {
    ...
    return db('sensor_readings as sr')
      ...
      .join(latestReadingTimes, function joinLatestReadings() {
        this.on('sr.sensor_id', '=', 'latest_readings.sensor_id').andOn('sr.time', '=', 'latest_readings.max_time');
      });
  }
  ...
  async getLatestActuatorActions() {
    ...
    return db('actuator_actions as aa')
      ...
      .join(latestActionTimes, function joinLatestActions() {
        this.on('aa.actuator_id', '=', 'latest_actions.actuator_id').andOn('aa.timestamp', '=', 'latest_actions.max_time');
      });
  }

  buildPlcSnapshot(plc, plcSensors, plcActuators, latestSensorById, latestActuatorById, snapshotTime) {
    ...
    const latestResponseDate =
      allResponseTimes.length > 0
        ...
        : null;
    const responseAgeMs = latestResponseDate ? snapshotTime.getTime() - latestResponseDate.getTime() : null;
  ...
  buildSensorSnapshot(sensor, latestReading, plcs, snapshotTime) {
    ...
    const plcConnected = sensor.plc_id ? plcManager.isConnected(sensor.plc_id) : false;
    const lastResponseDate = latestReading?.time ? new Date(latestReading.time) : null;
    const responseAgeMs = lastResponseDate ? snapshotTime.getTime() - lastResponseDate.getTime() : null;
    const freshnessWindowMs = getFreshnessWindowMs(plc?.scan_rate_ms);
  ...
  buildActuatorSnapshot(actuator, latestAction, plcs, snapshotTime) {
    ...
    const plcConnected = actuator.plc_id ? plcManager.isConnected(actuator.plc_id) : false;
    const lastResponseDate = latestAction?.timestamp ? new Date(latestAction.timestamp) : null;
    const responseAgeMs = lastResponseDate ? snapshotTime.getTime() - lastResponseDate.getTime() : null;
  ...
  async getLatestSnapshotSummary() {
    ...
    if (!latestSnapshotRow) {
      return { snapshot_time: null, snapshots: [], summary: { total: 0, green: 0, yellow: 0, red: 0, overall_status: 'yellow' } };
    }
    ...
    return {
      ...
      summary: {
        ...
        overall_status: getStatusFromRows(snapshots.map((item) => item.status), 'yellow')
      }
    };
  }
}
